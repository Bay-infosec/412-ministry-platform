...
| 2026-06-07 | Account 1 | **UI fixes + system group sync rewrite + member list views** (commit `fb37398`): (1) **Chat button moved to Home header** — Chat tab removed from BottomNav (was conflicting with the Updates bell icon); now a chat-bubble SVG button in the top-right of the Home header, shows red unread dot when `chatUnread` is true. App.jsx BottomNav wiring simplified (removed chat special-case, removed `chatUnread` prop from BottomNav). (2) **412 Board / Pastors sync bug fixed** — root cause: `syncGroup` was using `data.allProfiles` from App.jsx which is stale after tagging someone (PersonDetail writes tags to DB but doesn't trigger `loadData`). Fix: each group config now has an async `fetchMembers()` that queries the DB fresh on every sync, so tagging someone and hitting "Sync members" correctly picks them up. (3) **SystemGroups expandable member lists** — each group card is now a clickable header that expands/collapses a member list showing all current `conversation_participants` (pill badges with avatar + name). Member list refreshes after sync. (4) **Chat group member list** — group thread header now has a people-icon button; tapping it shows a horizontal avatar strip of all group members with first-name labels. Subtitle also shows live member count. (5) **Onboarding contact icons** — replaced emoji (📞 ✉️ ⛪) with inline SVGs (`#FF4D00` stroke) on the co-leader reveal card, matching the Bold Light design formula. All changes build clean, committed and pushed. |
| 2026-06-08 | Account 1 | **UI polish — events browser, event editor, profile, admin event detail** (commit `1391044`): (1) **EventsBrowser**: status label for active events changed from casual "Happening now" → "Active"; "You're in" green pill removed and replaced with a subtle inline checkmark SVG + "Enrolled" text (no pill box). (2) **EventEditor (new event creation)**: date range picker changed from a side-by-side grid (caused overflow on narrow screens) to a vertical stack — "Start date" and "End date" each take full width, no more overflow. (3) **EventHome**: onboarding progress banner added at the top of the enrolled event view (shows step N of 6, orange progress bar, "Continue Setup" + "View Checklist" buttons — visible only when `eventMember` exists and `!onboardingComplete` and viewing the active event); gear icon removed from dark event header card; "My Teams" → "Coordinator Dashboard". (4) **Profile**: PWA install instructions card added (dark #111111 card with numbered iPhone Safari + Android Chrome steps) placed before the push notification toggle. (5) **EventDetail full redesign**: hero card now has a prominent full-width orange "Edit Event" button; secondary actions (Publish/Archive/Duplicate/Delete) are small ghost chips below it; old "Conference Setup" section removed; edit popup restructured into "Event Details" section + "Conference Structure" section (coordinator count stepper → coordinator picker slots with search/avatar → team number chips with add/remove); new "Team Setup" section shows each team with EM/MM/BOTH ministry toggle + current leader chips + "+ Add leader" button (opens people picker from allProfiles). `saveEditModal` syncs coordinator event_members on save. Build clean, pushed, Vercel deploying. |
